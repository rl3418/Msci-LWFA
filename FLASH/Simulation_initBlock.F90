...
#include "Flash.h"
  ...
  real :: tradActual
  real :: rho, tele, trad, tion, zbar, abar, ye
  integer :: species
        ...
        do i = blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           if (sim_initGeom == "slab") then
              ...
              elseif(NDIM == 2 .or. NDIM == 3) then
                 if ( xcent(i) <= (FWHM/2.3548200450309493) .and. &
           ...
           if(species == TARG_SPEC) then
              rho = peak_rho
              tele = (sim_teleTarg-sim_teleCham)*EXP(-0.5*(xcent(i)/((FWHM/2.3548200450309493)))**2.0)+sim_teleCham
              tion = sim_tionTarg
              trad = sim_tradTarg
